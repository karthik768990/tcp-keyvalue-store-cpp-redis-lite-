...
static void heap_down(HeapItem *a,size_t pos,size_t len){
    HeapItem t = a[pos];
    while(true){
        //find teh smallest one among te parent and their kids
...
void heap_update(HeapItem *a,size_t pos , size_t len){
    if(pos>0 && a[heap_parent(pos)].val > a[pos].val) heap_up(a,pos);
